  ...
  return (
    <>
      ...
      >
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-2">
          ...
          <div className="flex lg:hidden items-center justify-between h-16">
            ...
            <Link href="/" className="relative group">
              <motion.div whileHover={{ scale: 1.02 }} className="flex flex-col items-center relative">
          ...
          <div className="hidden lg:grid lg:grid-cols-3 items-center h-20">
            ...
            <div className="flex items-center gap-6">
              <Link href="/" className="relative group">
                <motion.div whileHover={{ scale: 1.02 }} className="flex flex-col items-start relative">
                        ...
                        <div className="bg-[#0a0a0a]/95 backdrop-blur-xl border border-white/10 rounded-lg shadow-2xl shadow-black/50 overflow-hidden min-w-[600px]">
                          <div className="p-8">
                            <div className="flex items-center justify-between mb-6">
                              <span className="text-[10px] tracking-[0.3em] text-white/40 uppercase">Browse Categories</span>
                            ...
                            <div className="grid grid-cols-3 gap-8">
                              {categoryTree.map((category, idx) => (
                                <motion.div
                                  ...
                                  className="group/cat"
                                >
                                  <Link href={"/products/category/" + category.slug} className="block mb-4">
                                    <span className="text-sm font-medium text-white/90 group-hover/cat:text-white transition-colors flex items-center gap-2">
                                      {category.name}
                                      <ChevronRight size={12} className="opacity-0 -translate-x-2 group-hover/cat:opacity-100 group-hover/cat:translate-x-0 transition-all duration-300" />
                                    </span>
                                  </Link>
                          ...
                          <div className="bg-white/5 px-8 py-4 flex items-center justify-between">
                            <span className="text-[10px] text-white/30 tracking-wider">{categories.length} Categories • New arrivals weekly</span>
                            <Link href="/products" className="text-[11px] uppercase tracking-wider text-primary-foreground hover:text-white transition-colors flex items-center gap-2 group">
                              View All
            ...
            <div className="flex items-center justify-end gap-2">
              ...
              <motion.div whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
                <Link href="/wishlist" className="relative w-10 h-10 flex items-center justify-center rounded-full hover:bg-white/5 transition-colors text-white/70 hover:text-white group" aria-label="Wishlist">
                  <Heart size={18} className="transition-transform group-hover:scale-110" />
                  <AnimatePresence>
                    {wishlistItems.length > 0 && (
                      <motion.span initial={{ scale: 0 }} animate={{ scale: 1 }} exit={{ scale: 0 }} className="absolute -top-1 -right-1 w-4 h-4 bg-white text-black text-[9px] font-bold rounded-full flex items-center justify-center">
                        {wishlistItems.length}
                      </motion.span>
                    )}
                  </AnimatePresence>
              ...
              <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} onClick={openCart} className="relative w-10 h-10 flex items-center justify-center rounded-full hover:bg-white/5 transition-colors text-white/70 hover:text-white group" aria-label="Cart">
              ...
              <motion.div whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
                <Link href={isAuthenticated ? "/profile" : "/login"} className="relative w-10 h-10 flex items-center justify-center rounded-full hover:bg-white/5 transition-colors overflow-hidden group" aria-label="Profile">
                  {user?.avatar ? (
                    <div className="w-full h-full relative">
                      <Image src={user.avatar} alt={user.name || "User avatar"} fill sizes="40px" className="object-cover" onError={(e) => { (e.target as HTMLImageElement).style.display = "none"; }} />
                    </div>
                  ) : (
                    <User size={18} className="text-white/70 group-hover:text-white transition-colors" />
                  )}
                  {isAuthenticated && <div className="absolute bottom-1 right-1 w-2 h-2 bg-green-500 rounded-full border-2 border-[#0a0a0a]" />}
                </Link>
              </motion.div>
            </div>
          </div>
        ...
        <AnimatePresence>
          {isSearchOpen && (
            <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }} className="border-t border-white/10 bg-[#0a0a0a]/95 backdrop-blur-xl">
              <div className="max-w-3xl mx-auto px-6 py-6">
                <div className="relative">
                  <Search size={20} className="absolute left-4 top-1/2 -translate-y-1/2 text-white/30" />
                  <input type="text" placeholder="Search collections, products..." className="w-full bg-white/5 border border-white/10 rounded-full py-3 pl-12 pr-4 text-white placeholder:text-white/30 focus:outline-none focus:border-white/30 transition-colors text-sm" autoFocus />
                  <button onClick={() => setIsSearchOpen(false)} className="absolute right-4 top-1/2 -translate-y-1/2 text-white/30 hover:text-white transition-colors"><X size={16} /></button>
                </div>
              </div>
            </motion.div>
          )}
                ...
                <nav className="space-y-1">
                  {navLinks.map((link, index) => (
                    <motion.div key={link.href + link.label} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: index * 0.1 }}>
                      <Link href={link.href} onClick={() => setIsMobileMenuOpen(false)} className="flex items-center justify-between py-4 text-lg font-light text-white/80 hover:text-white border-b border-white/5 transition-colors">
                        <span className="tracking-wider">{link.label}</span>
                        {link.hasDropdown && <ChevronRight size={16} className="text-white/40" />}
                      </Link>
                      {link.hasDropdown && (
                        <div className="pl-4 py-2 space-y-2">
                          {categories.map((cat, idx) => (
                            <motion.div key={cat.id} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.3 + idx * 0.05 }}>
                              <Link href={"/products/category/" + cat.slug} onClick={() => setIsMobileMenuOpen(false)} className="block py-2 text-sm text-white/50 hover:text-white/80 transition-colors">{cat.name}</Link>
                            </motion.div>
                          ))}
                        </div>
                      )}
